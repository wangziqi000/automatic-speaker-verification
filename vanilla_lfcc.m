...
for cnt = 1:length(myFiles)
    ...
    try
        ...
        featureDict(myFiles{cnt}) = mean([stat,delta,double_delta],1);
    catch
...
trainFeatures = zeros(length(trainLabels),3*No_Filter);
...
fileList2 = myData{2};
testLabels = myData{3};
testFeatures = zeros(length(testLabels),3*No_Filter);

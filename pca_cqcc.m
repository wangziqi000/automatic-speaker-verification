...
fclose(fid);
myFiles = myData{1};
wholeFeatures = zeros(length(myFiles), 60);
...
% save('featureDictPCACQCC');
% load('featureDictPCACQCC.mat');
%%
...
trainFeatures = zeros(length(trainLabels),20);
...
fileList2 = myData{2};
testLabels = myData{3};
testFeatures = zeros(length(testLabels),20);
